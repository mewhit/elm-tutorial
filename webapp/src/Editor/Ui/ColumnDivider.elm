module Editor.Ui.ColumnDivider exposing (Model, Msg, init, isRightMost, openLeft, pushLeft, pushRight, update, view)
...
pushRight window px model =
    let
        newPx =
            toFloat (window.width - px) * (model.percent / 100)
    in
...
pushLeft window px model =
    let
        oldPx =
            toFloat window.width * (model.percent / 100)

        newPerc =
            oldPx * 100 / toFloat (window.width - px)
    in
...
rightMost window =
    100
        - toPercentage window
            (if Window.isMobile window then
                25
...
jump window percent =
    if percent >= rightMost window then
        leftMost window

    else if percent <= leftMost window then
        if Window.isMobile window then
            rightMost window
    ...
    else if percent >= halfPoint then
        rightMost window
        ...
        OnMove latest ->
            case model.movement of
                Moving percent initial False ->
                    if isSignificant initial latest then
                        { model | percent = toPercentage window latest, movement = Moving percent initial True }
...
viewLeft onMsg window model percent =
    ...
    div <|
        [ id "left-side"
        , style "width" (String.fromFloat 100.0 ++ "%")
...
viewDivider window model percent =
    node "column-divider"
        ...
        , style "width"
            (if isRightMost window model then
                "40px"
