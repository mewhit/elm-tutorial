module Editor.Data.Status exposing
    ( Status(..)
    , changed
    , compiling
    , getProblems
...
import Editor.Data.Problem as Problem exposing (Problem, Problems)
...
getProblems status =
    case status of
        ...
        Failed _ ->
            Just ( [], Problem.generic, [] )

        HasProblems pbs ->
            Just pbs

        HasProblemsButChanged pbs ->
            Just pbs

        HasProblemsButRecompiling pbs ->
            Just pbs
...
hasProblems status =
    case status of
        ...
        HasProblemsButChanged _ ->
            True

        HasProblemsButRecompiling _ ->
            True
...
withProblems status func =
    case status of
        ...
        HasProblems pbs ->
            HasProblems (func pbs)

        HasProblemsButChanged pbs ->
            HasProblemsButChanged (func pbs)

        HasProblemsButRecompiling pbs ->
            HasProblemsButRecompiling (func pbs)
...
changed status =
    case status of
        ...
        Failed _ ->
            HasProblemsButChanged ( [], Problem.generic, [] )
...
compiling status =
    case status of
        ...
        Failed _ ->
            HasProblemsButRecompiling ( [], Problem.generic, [] )
    ...
    case D.decodeValue Error.decoder value of
        Ok errors ->
            case Problem.init (Problem.toIndexedProblems errors) of
                ...
                Nothing ->
                    Failed "Somehow returned zero problems."
