module Editor.Data.Time exposing
    ( ceilingDay
    , ceilingHour
    , ceilingMinute
    , ceilingMonth
...
floorMs zone mult stamp =
    let
        parts =
            T.posixToParts zone stamp

        rem =
            remainderBy mult parts.millisecond
    in
    if rem == 0 then
        T.partsToPosix zone parts

    else
        T.add T.Millisecond -rem zone stamp
...
floorSecond zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.floor T.Second zone stamp)

        rem =
            remainderBy mult parts.second
...
floorMinute zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.floor T.Minute zone stamp)

        rem =
            remainderBy mult parts.minute
...
floorHour zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.floor T.Hour zone stamp)

        rem =
            remainderBy mult parts.hour
...
floorMonth zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.floor T.Month zone stamp)

        monthInt =
            monthAsInt parts.month

        rem =
            remainderBy mult (monthInt - 1)
...
floorYear zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.ceiling T.Year zone stamp)

        rem =
            remainderBy mult parts.year
...
ceilingMs zone mult stamp =
    ...
    if rem == 0 then
        ...
        T.add T.Millisecond (mult - rem) zone stamp
...
ceilingSecond zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.ceiling T.Second zone stamp)
    ...
    if rem == 0 then
        ...
        T.add T.Second (mult - rem) zone new
...
ceilingMinute zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.ceiling T.Minute zone stamp)
    ...
    if rem == 0 then
        ...
        T.add T.Minute (mult - rem) zone new
...
ceilingHour zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.ceiling T.Hour zone stamp)
    ...
    if rem == 0 then
        ...
        T.add T.Hour (mult - rem) zone new
...
ceilingMonth zone mult stamp =
    let
        parts =
            T.posixToParts zone (T.ceiling T.Month zone stamp)
    ...
    T.partsToPosix zone <|
        if newMonth > 12 then
            { parts | year = parts.year + 1, month = intAsMonth (newMonth - 12) }

        else
            { parts | month = intAsMonth newMonth }
