module Editor.Data.Problem exposing
    ( Location(..)
    , Problem
    , Problems
    , focusNext
...
focusNext pbs =
    case pbs of
        ( prev, curr, next :: remaining ) ->
            ( curr :: prev, next, remaining )
...
focusPrevious pbs =
    case pbs of
        ( prev :: remaining, curr, next ) ->
            ( remaining, prev, curr :: next )
...
getSummary problem =
    let
        ...
        firstColon =
            List.head (String.split ":" plainText)
    in
    case ( firstLine, firstColon ) of
        ( Just firstLine_, Just firstColon_ ) ->
            if String.length firstLine_ > String.length firstColon_ then
                firstColon_
