module Editor.Data.Hint exposing
    ( Hint
    , Info(..)
    , Table
    , buildTable
...
keywordsAndConventions =
    let
        ...
        if_ =
            hint "If Expressions" "https://guide.elm-lang.org/core_language.html#if-expressions"

        tea_ =
            hint "The Elm Architecture" "https://guide.elm-lang.org/architecture/"

        subs_ =
            hint "Subscriptions" "https://guide.elm-lang.org/effects/"

        case_ =
            hint "Pattern Matching" "https://guide.elm-lang.org/types/pattern_matching.html"

        import_ =
            hint "Imports" "https://github.com/elm/compiler/blob/master/hints/imports.md"
    in
    Dict.fromList
        [ ( "if", if_ )
        , ( "then", if_ )
...
buildTable (Header.Imports imports) depsInfo =
    Table <|
        Dict.union keywordsAndConventions <|
    ...
    case Dict.get key table of
        ...
        Just info ->
            case info of
                ...
                Specific { text, href } ->
                    if hint.text == text && hint.href == href then
                        table

                    else
                        Dict.insert key Ambiguous table
...
addQualified moduleName maybeAlias moduleUrl info hints =
    ...
    in
    ( "module:" ++ moduleName, Hint moduleName moduleUrl )
        :: hints
