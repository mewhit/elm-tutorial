module Editor.Data.Header exposing
    ( Exposed
    , Exposing(..)
    , Import
    , Imports(..)
...
addItem item exposed =
    case item of
        Op name ->
            { exposed | ops = Set.insert name exposed.ops }

        Lower name ->
            { exposed | lower = Set.insert name exposed.lower }

        Upper name bool ->
            { exposed | upper = Dict.insert name bool exposed.upper }
