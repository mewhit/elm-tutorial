module Editor.Data.Registry exposing
    ( Action(..)
    , Registry
    , attemptEdit
    ,  dismissAll
...
attemptEdit action toMsg registry package =
    ...
    Http.riskyRequest
        ...
        , expect = Editor.Data.Http.expectJson toMsg Error.decoder Solution.decoder
        , timeout = Nothing
        , tracker = Nothing
        }
...
listStep ( n, pkgs ) =
    if n <= 0 then
        D.succeed (D.Done pkgs)

    else
        D.map (\pkg -> D.Loop ( n - 1, pkg :: pkgs )) decodeOne
...
decodeVersion =
    let
        decodeMinorAndPatch firstNumber =
